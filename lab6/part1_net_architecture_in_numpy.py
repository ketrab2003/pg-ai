...
def zad1_single_neuron(student_id):
    ...
    # ustawienie właściwych wag (0.5 point)
    model.W[:, 0] = [-1, 0]
    model.b[:] = [0]
...
def zad2_two_layer_net(student_id):
    ...
    class DenseLayer:
        ...
        def forward(self, x_data: np.ndarray) -> np.ndarray:
            """
            :param x_data: wejście warstwy: np.array o rozmiarze [n_samples, n_in]
            :return: wyjście warstwy: np.array o rozmiarze [n_samples, n_out]
            """
            x_data = x_data.dot(self.W) + self.b
            return self.f_act(x_data)
    ...
    class SimpleTwoLayerNetwork:
        ...
        def forward(self, x_data: np.ndarray) -> np.ndarray:
            """
            :param x_data: wejście modelu: np.array o rozmiarze [n_samples, n_in]
            :return: wyjście modelu: np.array o rozmiarze [n_samples, n_out]
            """
...
if __name__ == '__main__':
    ...
    student_id = 193319         # Twój numer indeksu, np. 102247
